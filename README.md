...
| ✅ Live | `rrr` (`ak.macro_china_reserve_requirement_ratio`) — previously marked broken, which was a misdiagnosis: the fetcher was searching for column names that don't exist in this table (`日期`/`TRADE_DATE` instead of the real `生效时间`/`大型金融机构-调整后`), so it silently raised on every run. Fixed. | current through 2025-05 — cross-checked directly against [PBOC's own announcement archive](https://www.pbc.gov.cn/en/3688229/3688335/3730270/index.html), which confirms May 2025 really is the latest RRR action (not staleness); RRR only changes on PBOC action, not monthly |
